...
export const reducer = (state, action) => {
	...
	switch (type) {
		...
		case 'activityData':
			...
			Base.setSensitiveInfo('common.activityData', data);
			break;
		case 'walletsList':
			const list = data || [];
			Base.setSensitiveInfo('common.walletsList', list);
...
export const useUpdateBalance = () => {
	...
	const updateBalance = (balance, address) => {
		...
		balance = new BigNumber(balance).div(IotaSDK.IOTA_MI);
		const assets = price.IOTA ? balance.times(new BigNumber(price.IOTA || 0)) : 0;
		const list = [
			{
		...
		const totalAssets = {
			iotaBalance: Base.formatNum(balance),
			iotaUnit: 'Mi',
			assets: Base.formatNum(assets)
		};
...
export const useGetAssetsList = (curWallet) => {
	...
	useEffect(async () => {
		...
		const [addressList, outputList] = await IotaSDK.getValidAddresses(curWallet);
		// Sync balance
		IotaSDK.getBalance(curWallet, addressList)
			.then((balance) => {
				updateBalance(balance, curWallet.address);
			})
			.catch(() => {
				setRequestAssets(true, dispatch);
				setAssetsData({}, [], dispatch);
...
export const useCreateCheck = (callBack) => {
	...
	const [__, canUseWalletsList] = useGetNodeWallet();
	const curNodeId = _get(store, 'common.curNodeId');
	useEffect(() => {
